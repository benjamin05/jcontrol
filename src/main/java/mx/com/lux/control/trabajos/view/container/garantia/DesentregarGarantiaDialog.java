...
public class DesentregarGarantiaDialog extends Dialog {

	private final Logger log = LoggerFactory.getLogger( DesentregarGarantiaDialog.class );
	...
	public DesentregarGarantiaDialog( final Shell parentShell, final String rx ) {
		super( parentShell );
		shell = parentShell;
	...
	protected void configureShell( Shell newShell ) {
		...
		newShell.setText( "Desentregar Garantía" );
		shell = newShell;
	}
		...
		botonGuardar.addSelectionListener( new SelectionListener() {
			...
			public void widgetSelected( SelectionEvent event ) {
				if ( sonDatosValidos() ) {
					...
					try {
						if( garantiaService.desentregarGarantia( rx, textoObservaciones.getText(), empleado.getIdEmpleado() ) ) {
							MessageDialog.openInformation( shell, "Desentregar Garantia", "Se ha procesado la Garantía correctamente" );
